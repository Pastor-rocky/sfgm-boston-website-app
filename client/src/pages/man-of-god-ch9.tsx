...
export default function ManOfGodCh9() {
  const [isPlaying, setIsPlaying] = useState(false);
  const [currentTime, setCurrentTime] = useState(0);
  const [duration, setDuration] = useState(0);
  const [volume, setVolume] = useState(1);
  const audioRef = useRef<HTMLAudioElement>(null);
  const chapter = getManOfGodChapter(9)!;
        ...
        <Card className="mb-8 bg-gradient-to-r from-emerald-700 to-emerald-900 border-none shadow-2xl">
          <CardContent className="p-6">
            ...
            <div className="flex items-center justify-center gap-4 mb-4">
              <Button onClick={() => skip(-15)} size="sm" variant="ghost" className="text-white hover:bg-white/20">
            ...
            <div className="flex items-center gap-3 mt-4">
              ...
              <Slider value={[volume]} max={1} step={0.01} onValueChange={handleVolumeChange} className="w-24 cursor-pointer" />
            </div>
